...
class SegmentationTrainer:
    def __init__(self, config_path="../configs/experiment_config.yaml", keep_temp: bool = False):
        ...
        # Get the evaluation_training directory as base
        script_dir = Path(__file__).parent  # training/
        self.base_dir = script_dir.parent   # evaluation_training/
        ...
        # Set directories relative to evaluation_training using config
    ...
    def _ensure_val_pseudo_labels(self, training_data_dir: Path, organ_type: str, tumor_type: str) -> None:
        ...
        try:
            ...
            if not val_list.exists():
                return
            step3_root = (self.base_dir.parent / "utility_training_resources" / "DiffTumor" / "STEP3.SegmentationModel").resolve()
    ...
    def prepare_training_data(self, dataset, method, model_type):
        ...
        if not real_data_dir.exists():
            # Fallback to legacy utility_training_resources datasets
            fallback = self.base_dir.parent / "utility_training_resources" / "datasets" / ("LIDC_real" if dataset == "lidc" else "EMIDEC_real")
            if fallback.exists():
    ...
    def train_model(self, dataset, method, model_type, seg_model, checkpoint_path: Path | None = None):
        ...
        diff_tumor_main = (self.base_dir.parent / "utility_training_resources" / "DiffTumor" / "STEP3.SegmentationModel" / "main.py").resolve()
        cmd = [
        ...
        try:
            ...
            rc = self._run_streaming(
                cmd=cmd,
                cwd=str((self.base_dir.parent / "utility_training_resources" / "DiffTumor" / "STEP3.SegmentationModel").resolve()),
                timeout=None,
                log_path=log_path,
            )
            if rc == 0:
